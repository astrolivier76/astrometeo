...
const unsigned long ECONOMY_CHECK_INTERVAL = 300000; // Vérif toutes les 5min
...
#define WIFI_POWER_LOW 62     // 14dBm - puissance réduite
...
#define INTERO_VENT 20        // 20s
#define INTERO_PLUVIO 360     // 6min  
#define INTERO_GOUTTES 60     // 60s
//#define CACHED_INTERVAL 20000 // 20s
...
void setupUnusedPins() {
    const int unusedPins[] = {4, 12, 13, 14, 15, 25, 26, 27, 32, 33};
...
void setup() {
    ...
    server.on("/api/control", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        String response = "{\"mqtt\":\"" + String(mqttEnabled ? "enabled" : "disabled") + "\",\"websocket\":\"" + String(websocketEnabled ? "enabled" : "disabled") + "\"}";
...
void loop() {
    ...
    // Chauffage toutes les 10 min
    #ifdef CHAUFFAGE
     #ifdef SHT
      if (currentMillis - t_lastActionChauffage >= INTERO_CHAUFFAGE * 1000) {
          t_lastActionChauffage = currentMillis;
          chauffage = needChauffage_SHT();
          digitalWrite(pinChauffage, chauffage == 1 ? HIGH : LOW);
      }
      #else
      if (currentMillis - t_lastActionChauffage >= INTERO_CHAUFFAGE * 1000) {
          t_lastActionChauffage = currentMillis;
          chauffage = needChauffage_BME280();
          digitalWrite(pinChauffage, chauffage == 1 ? HIGH : LOW);
      }
      #endif      
    #endif
...
void updateSensorCache() {
    ...
    if (xSemaphoreTake(xSensorDataMutex, portMAX_DELAY) == pdTRUE) {
        ...
        lux = getLux(); // TSL2591 gère déjà les erreurs
        sqm = getSQM(); // TSL2591 gère déjà les erreurs
        #else
        updateBME();
        updateMLX();
        updateTSL2591();            
